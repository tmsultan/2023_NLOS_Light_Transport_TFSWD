...
%% Create Folder
fname = '../Figures/Results_Occluded_RSD/';
if ~exist(fname)
    mkdir(fname)
end
...
%% Apply the mask at plane B -- Double Slit Target
% mask=zeros(1,samples);
% 
% % Slit 1
% ind1 = find(min(abs(xB - 0.26)) == abs(xB-0.25));
% ind2 = find(min(abs(xB - 0.27)) == abs(xB-0.27));
% 
% % Slit 2
% ind3 = find(min(abs(xB + 0.25)) == abs(xB+0.25));
% ind4 = find(min(abs(xB + 0.27)) == abs(xB+0.27));
% 
...
zC=zB+1.18;%z locaton of destination plane (source plane is at z=0)
...
xlabel('x position (cm)'); ylabel('Normalized E-Field'); legend('Real Part', 'Imaginary Part');
subplot(1,2,2)
plot(xC.*100,abs(EC.^2)); title('Intensity at plane C'); xlabel('x position (cm)'); ylabel('Normalized Intensity') %plot intensity at plane C

%% Apply the mask at plane C - Occluder
OccMask=ones(1,samples);

OccWidth = 0.105;

% Slit 1
ind1 = find(min(abs(xC - 0)) == abs(xC-0));
ind2 = find(min(abs(xC - OccWidth/2)) == abs(xC-OccWidth/2));

% Slit 2
ind3 = find(min(abs(xC + 0)) == abs(xC+0));
ind4 = find(min(abs(xC + OccWidth/2)) == abs(xC+OccWidth/2));

OccMask(ind1:ind2)=0; % mask goes from -1.5 mm to +1.5 mm
OccMask(ind4:ind3)=0; % mask goes from -1.5 mm to +1.5 mm
figure; plot(xC,OccMask); title('Occluder tansmission'); xlabel('x position (cm)');

EC_mask=EC.*OccMask; % Propagate for entire EB and then apply mask
%% plot the result
figure; 
subplot(1,2,1)
hold on;
plot(xC.*100,real(EC_mask));
plot(xC.*100,imag(EC_mask));
hold off;
title('E-Field at plane C'); 
xlabel('x position (cm)'); ylabel('Normalized E-Field'); legend('Real Part', 'Imaginary Part');
subplot(1,2,2)
plot(xC.*100,abs(EC_mask.^2)); title('Intensity at plane C'); xlabel('x position (cm)'); ylabel('Normalized Intensity') %plot intensity at plane B

%% %% Propagate from plane C to plane D
xD=linspace(-width,width,samples);%x coordinate for the sample points in plane C
samples1 = length(xD);

ED=zeros(1,samples1)+zeros(1,samples1)*1i;

% C is 10 cm away from B
zD=zC+0.77;%z locaton of destination plane (source plane is at z=0)

disp('Propagating C -> D ...')

for l=1:samples
    r=sqrt((xD-xC(l)).^2+(zD-zC)^2);%Distance between source and sample points
    ED=ED+EC_mask(l).*exp(1i*k*r)./r*dx; %integral (note the difference between 1 and l)
end

ED=ED./max(ED);%re-normalizing. Let's not worry about the coefficients
...
plot(xD.*100,imag(ED));
hold off;
title('E-Field at plane D - Image'); 
xlabel('x position (cm)'); ylabel('Normalized E-Field'); legend('Real Part', 'Imaginary Part');
subplot(1,2,2)
plot(xD.*100,abs(ED.^2)); title('Intensity at plane D'); xlabel('x position (cm)'); ylabel('Normalized Intensity') %plot intensity at plane B
...
hold off;
title('E-Field at plane B after mask'); 
xlabel('x position (cm)'); ylabel('Normalized E-Field'); 
...
% Plot the E-Field at C before Mask
subplot(6,2,7)
hold on;
plot(xC.*100,real(EC));
plot(xC.*100,imag(EC));
...
% Plot the E-Field at C after mask
subplot(6,2,9)
hold on;
plot(xC.*100,real(EC_mask));
plot(xC.*100,imag(EC_mask));
hold off;
title('E-Field at plane C after mask'); 
xlabel('x position (cm)'); ylabel('Normalized E-Field'); 
legend('Real Part', 'Imaginary Part');
subplot(6,2,10)
plot(xC.*100,abs(EC_mask.^2)); 
title('Intensity at plane C after mask' ); 
xlabel('x position (cm)'); ylabel('Normalized Intensity') %plot intensity at plane B
...
plot(xD.*100,imag(ED));
hold off;
title('E-Field at plane D'); 
xlabel('x position (cm)'); ylabel('Normalized E-Field'); legend('Real Part', 'Imaginary Part');
subplot(6,2,12)
plot(xD.*100,abs(ED.^2)); title('Intensity at plane D'); xlabel('x position (cm)'); ylabel('Normalized Intensity') %plot intensity at plane B

% ED2 = fft(ED)
% figure; plot(abs(fftshift(ED2)))
...
figure
plot(xA1.*100, abs(ED.^2)./(abs(ED(samples/2 + 1).^2)),'LineWidth', LW)
hold on; 
MSG = ['Radio Wave ' , newline ,  num2str(F5/10^9), ' GHz/', num2str(lambda*100), ' cm Wavelength'];
...
hold off
saveas(gcf, [fname, 'Occ_4_cm_RSD_ExpCopy'], 'png');
savefig(gcf, [fname, 'Occ_4_cm_RSD_ExpCopy']);
% set(gcf, 'color', 'none');   
% set(gca, 'color', 'none');

save('../Results_Mat_Files/Simulation_Results_PF_Occluded', 'ED', 'xA', 'xA1', 'samples');
